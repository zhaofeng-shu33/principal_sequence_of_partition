...
namespace submodular {
    ...
    TEST_F(PINModelTest, DGraphBase) {
        SimpleGraph<double> sg = make_dgraph(n_1, edge_list_float_1);
        // Test Node Access
        SimpleGraph<double>::Node_s ns[3] = { sg.GetNode(0), sg.GetNode(1), sg.GetNode(2) };
        EXPECT_EQ(ns[0]->name, 0);
        EXPECT_EQ(ns[0]->excess, 0);
        // Test Node Iterator
        SimpleGraph<double>::node_range nr = sg.NodeRange();
        for (const auto&  it : nr) {
            continue;
        }
        // Test Arc Access
        SimpleGraph<double>::Arc_s as[3] = { sg.GetArc(1, 0), sg.GetArc(2, 1), sg.GetArc(2, 0)};
        EXPECT_EQ(as[2]->capacity, 5);
        EXPECT_EQ(as[2]->tail_node.lock()->name, 0);
        EXPECT_EQ(as[2]->head_node.lock()->name, 2);
        // Test Arc Iterator
        SimpleGraph<double>::arc_range ar = sg.OutArcRange(0);
        double sum = 0;
    ...
    TEST_F(PINModelTest, DGraphCut) {
        SimpleGraph<double> sg = make_dgraph(n_1, edge_list_float_1);
        DirectedGraphCutOracle<double> dgc(sg);
