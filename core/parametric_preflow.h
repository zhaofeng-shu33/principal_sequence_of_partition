 ...
 *
 * This file is modified from LEMON "preflow.h" by zhaofeng-shu33
 *
...
#ifndef PARAMETRIC_PREFLOW_H
#define PARAMETRIC_PREFLOW_H
...
namespace lemon {
  ...
  public:
    ...
    /// \brief Sets the excess map.
    ///
    /// Sets the excess map.
    /// \return <tt>(*this)</tt>
    Preflow& excessMap(const ExcessMap& map) {
        _excess = &map;
        return *this;
    }
    ...
    /// \brief Returns a const reference to the excess.
    ///
    /// Returns a const reference to the excess.
    ///
    /// \pre Either \ref run() or \ref init() must be called before
    /// using this function.
    const ExcessMap& excess() const {
        return *_excess;
    }
    ...
    /// \brief update the \c ExcessMap
    ///
    /// update the \c ExcessMap from \c flowMap
