...
namespace psp {
	...
	DilworthTruncation_Improved::DilworthTruncation_Improved(double lambda, Digraph * g, ArcMap * edge_map) :
		lambda_(lambda), min_value(0), _g(g), _edge_map(edge_map), xl(lemon::countNodes(*g), -lambda)
	{
		node_size = lemon::countNodes(*g);
	}

	double DilworthTruncation_Improved::get_min_value() {
		return min_value;
	}

	stl::Partition& DilworthTruncation_Improved::get_min_partition() {
		return _partition;
	}

	void DilworthTruncation_Improved::run() {
		min_value = 0;
		_partition.clear();		
		stl::CSet V_minus_U = stl::CSet::MakeDense(node_size);
		double alpha_l = 0;
		while(V_minus_U.Cardinality() > 0) {
			int i = *V_minus_U.rbegin(); // maximum value
			minimize(i);
			alpha_l = min_value;
			Tl.AddElement(i);
			_partition = _partition.expand(Tl);
			xl[i] = alpha_l;
			V_minus_U = V_minus_U.Difference(Tl);
		}
		min_value = 0;
		for (auto it = xl.begin(); it != xl.end(); it++) {
			min_value += *it;
	...
	double DilworthTruncation_Improved::evaluate(stl::Partition & partition) {
		double result = submodular::get_partition_value(*_g, *_edge_map, partition);
		return result - lambda_ * partition.Cardinality();
	}

	void DilworthTruncation_Improved::minimize(int i) {
		...
		int graph_size = lemon::countNodes(*_g);
		std::map<int, double> sink_node_cost_map;
		for (Digraph::InArcIt a(*_g, _g->nodeFromId(i)); a != lemon::INVALID; ++a) {
			int node_id = _g->id(_g->source(a));
			sink_node_cost_map[node_id] = (*_edge_map)[a];
		}
		Digraph::NodeMap<bool> filter(*_g, true);
		SubDigraph subgraph(*_g, filter);
		subgraph.disable(subgraph.nodeFromId(i));
		...
		for (int j = 0; j < graph_size; j++) {
			...
			double sink_cost = std::max<double>(0, xl[j]) + sink_node_cost_map[j];
			if (sink_cost > 0) {
				arc = subgraph.addArc(subgraph.nodeFromId(j), sink_node);
				(*_edge_map)[arc] = sink_cost;
			}
			const_difference += std::max<double>(0, xl[j]);
		}
